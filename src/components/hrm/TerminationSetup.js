...
import { createTermination } from "../../api/apicalls";
...
const TerminationSetup = () => {
  ...
  const getProjectQuery = useQuery(["cat"], () => getTerminations(), {
  ...
  async function getTerminations() {
    try {
      ...
      console.log("getTermination===>", response);
  ...
  async function createTermination() {
    setIsLoading(true);
    try {
      const response = await api.createTermination({
        name: formValue.name,
        type: formValue.type,
      });
      console.log("createTermination===>", response);
  ...
  async function deleteTermination(id) {
    ...
    setIsLoading(true);
    try {
      const response = await api.deleteTermination(id);
                ...
                getProjectQuery?.data?.data?.product_categories?.map(
                  (result) => (
                    <tr key={result.name} className="mb-2 hover:bg-light-gray">
                      ...
                      <td className="whitespace-nowrap py-[16px] bg-white  px-5  border-b-[0.8px] border-[#E4E7EC] text-[14px] leading-[24px] tracking-[0.2px] text-[#667185] font-medium text-left  ">
                        <button onClick={() => deleteTermination(result.id)}>
                          {" "}
