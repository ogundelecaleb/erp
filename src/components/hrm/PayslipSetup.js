...
import { createPayslip } from "../../api/apicalls";
...
const PayslipSetup = () => {
  ...
  const getProjectQuery = useQuery(["cat"], () => getPayslips(), {
  ...
  async function getPayslips() {
    try {
      ...
      console.log("getPayslip===>", response);
  ...
  async function createPayslip() {
    setIsLoading(true);
    try {
      const response = await api.createPayslip({
        name: formValue.name,
        type: formValue.type,
      });
      console.log("createPayslip===>", response);
  ...
  async function deletePayslip(id) {
    ...
    setIsLoading(true);
    try {
      const response = await api.deletePayslip(id);
                  ...
                  (result) => (
                    <tr key={result.name} className="mb-2 hover:bg-light-gray">
                      ...
                      <td className="whitespace-nowrap py-[16px] bg-white  px-5  border-b-[0.8px] border-[#E4E7EC] text-[14px] leading-[24px] tracking-[0.2px] text-[#1A202C] font-medium text-left  ">
                       ...
                       <input
                       type="color"
                          className="w-[60px] md:w-[90px] h-[24px] "
                          value={color}
                      ...
                      <td className="whitespace-nowrap py-[16px] bg-white  px-5  border-b-[0.8px] border-[#E4E7EC] text-[14px] leading-[24px] tracking-[0.2px] text-[#667185] font-medium text-left  ">
                        <button onClick={() => deletePayslip(result.id)}>
                          {" "}
